                            ...
                            <div class="container px-5 py-8 mx-auto">
                                ...
                                <table class="table-auto w-full text-left whitespace-no-wrap">
                                    ...
                                    <tr v-for="order in props.orders.data" :key="order.id">
                                        <td class="boder-gray-200 px-4 py-3">
                                            <Link class="text-blue-400" :href="route('purchases.show',{ purchase: order.id})">
                                                {{  order.id }}
                                            </Link>
                                        </td>
                                        <td class="boder-gray-200 px-4 py-3">{{ order.customer_name }}</td>
                                        <td class="boder-gray-200 px-4 py-3">{{ order.total }}</td>
                                        <td class="boder-gray-200 px-4 py-3">{{ order.status }}</td>
                                        <td class="boder-gray-200 px-4 py-3">{{ order.created_at}}</td>
                                    </tr>
                                    </tbody>
                                </table>
                                </div>
